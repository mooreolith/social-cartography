...
(function($){
  ...
  $.widget('jmm.social_cartography', {
    ...
    delete: function(vertex){
      Entity.all.splice(Entity.all.findIndex(v => v.id == vertex.entity.id), 1);
      Person.all.splice(Person.all.findIndex(p => p.id == vertex.entity.id), 1);
    ...
    edit: function(vertex){
      ...
      $('#delete-vertex').one('click', () => {
        switch(entity.type){
          case 'Person':
            this.remove_person(entity.id);
            break;
          case 'Group':
            this.remove_group(entity.id);
            break;
          case 'Role':
            this.remove_role(entity.id);
            break;
        }
      })
    },
    ...
    remove_person: function(id){
      ...
      Entity.all.splice(Entity.all.findIndex(e => e.id == id));
    },
      ...
      switch(typeof info.group){
        ...
        case 'object':
          if(!info.group.vertex){
            info.group = this.add_group(info.group);
          }
      }
    ...
    import: function(file){
      ...
      reader.onload = (e) => {
        ...
        var findMaxId = function(entities){
          var max_id = 0;
          entities.forEach(e => {
            if(e.id > max_id){
              max_id = e.id;
        ...
        var max_id = findMaxId(Entity.all);
        console.log("max_id", max_id)

        for(var i=0; i<input.persons.length; i++){
          console.log(input.persons[i].id, '--->', input.persons[i].id + max_id);
          input.persons[i].id += max_id;
        }

        for(var i=0; i<input.groups.length; i++){
          input.groups[i].id += max_id;
        }

        for(var i=0; i<input.roles.length; i++){
          input.roles[i].person += max_id;
          input.roles[i].group += max_id;
        }

        input.persons = input.persons.sort(byId);
        input.persons.forEach(p => p.type = 'person');
        input.persons.forEach(that.add_person);

        input.groups = input.groups.sort(byId);
        input.groups.forEach(g => g.type = 'group');
        input.groups.forEach(that.add_group);

        input.roles = input.roles.sort(byId);
        input.roles.forEach(r => r.type = 'role');
        input.roles.forEach(that.add_role);

        console.log('next Entity.id', Entity.id);
