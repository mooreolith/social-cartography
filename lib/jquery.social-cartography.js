...
(function($){
  ...
  var Role = function(info){
    ...
    this.name = `${info.person.name}@${info.group.name}`;
  ...
  $.widget('jmm.social_cartography', {
    ...
    _create: function(options){
      ...
      $('#role-description').on('keydown', event => {
        if(event.keyCode == 13) {
          ...
          if(!sub_component){
            sub_component = $('#display').social_cartography('add_person', {name: sub_name, texture: 'img/person.png'});
          }
          ...
          try{
            ...
            if(!super_component && super_name){
              super_component = $('#display').social_cartography('add_group', {name: super_name, texture: 'img/group.png'});
            }
          }catch(e){
            console.error(e);
          }
          ...
          if(sub_component && super_component){
            var role = $('#display').social_cartography('add_role', {'person': sub_component, 'group': super_component, texture: 'img/role.png'});
          }
    ...
    add_role: function(info){
      ...
      if(info.person !== undefined && info.group !== undefined){
        switch(typeof info.person){
          case 'number':
          ...
          case 'string':
            ...
            if(!info.person){
              info.person = Group.all.find(g => g.name == info.person);
            }
            if(!info.person){
        ...
        switch(typeof info.group){
          case 'number':
            info.group = Group.all.find(p => p.id == info.group);
            break
          case 'string':
            var group_name = info.group;
            info.group = Group.all.find(g => g.name == info.group);
            if(!info.group){
              info.group = Group.all.find(g => g.name == info.group);
            }
    ...
    import: function(file){
      ...
      reader.onload = (e) => {
        ...
        var all = [
          ...input.persons.sort(byId).map(p => Object.assign(p, {type: 'person'})),
          ...input.groups.sort(byId).map(g => Object.assign(g, {type: 'group'})), 
          ...input.roles.sort(byId).map(r => Object.assign(r, {type: 'role'}))
        ];
