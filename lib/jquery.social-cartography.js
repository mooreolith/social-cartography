...
(function($){
  ...
  var Role = function(person, group){
    ...
    this.name = `${person.name}@${group.name}`;
  ...
  Role.prototype.set = function(options){
    this.name = options.name !== undefined ? options.text : this.name;
    this.from = options.from !== undefined ? options.from : this.from;
    this.until = options.until !== undefined ? options.until : this.until;
  ...
  $.widget('jmm.social_cartography', {
    ...
    _create: function(options){
      ...
      $('#show-labels').change(function(){
        if($(this).is(':checked')){
          ...
          $('<style id="labels-hidden">.vertex-label { visibility: hidden; } </style>').appendTo('html > head');
        }
      })
    ...
    edit: function(vertex){
      ...
      $('#edit-vertex-name').val(entity.name);
      $('#edit-vertex-texture').val(vertex.texture);
      $('#edit-vertex-from').val(entity.from);
      $('#edit-vertex-until').val(entity.until);

      var edit_vertex = document.querySelector('#edit-vertex');
      edit_vertex.showModal();
      $('#confirm-edit-vertex').one('click', () => {
        entity.set({
          text: $('#edit-vertex-name').val(),
          texture: $('#edit-vertex-texture').val(),
          from: $('#edit-vertex-from').val(),
          until: $('#edit-vertex-until').val()
        });
      ...
      reader.onload = function(e){
        ...
        for(var i in input.groups){
          var group = $('#display').social_cartography('add_group', {
            ...
            texture: input.groups[i].texture
          });
        ...
        for(var i in input.persons){
          var person = $('#display').social_cartography('add_person', {
            ...
            texture: input.persons[i].texture
          });
